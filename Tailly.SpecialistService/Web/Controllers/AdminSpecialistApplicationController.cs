...
public class AdminSpecialistApplicationController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll([FromQuery] int page = 1, [FromQuery] int limit = 20, [FromQuery] string? status = "pending_review")
    {
        var pagination = new PaginationValidator(page, limit);

        var statusEnum = SpecialistEnumMapper.ParseApplicationStatus(status);
    ...
    public async Task<IActionResult> AssignInterview(Guid id, [FromBody] AssignInterviewRequest request)
    {
        ...
        var result = await _service.AssignInterviewAsync(id, request.Note, request.InterviewDate);
    ...
    [HttpPost("admin/specialist-applications/{id:guid}/attach-specialist-account")]
    public async Task<IActionResult> AttachSpecialistAccount(Guid id, [FromBody] AttachSpecialistAccountRequest request)
    {
        ...
        return Ok(new { success = true, specialistId = result.Value.Id });
    }
}